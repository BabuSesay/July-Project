...
export const Indicator = ({ indicator, onChange }) => {
  ...
  return (
    <div>
      ...
      {editing && <input ref={inputRef} disabled={loading} type="text" style={{ padding: 5, width: 300, textAlign: "center" }} defaultValue={indicator.displayName} />}
      {error && <span style={{ color: "red" }}>ERROR</span>}
      {!error && (
        <button onClick={editing ? doMutation : edit} disabled={loading}>
          {editing ? "Submit" : "Edit"}
        </button>
      )}
